...
-- All statements use IF NOT EXISTS; no DROP statements.
...
CREATE TABLE IF NOT EXISTS "DP_PROVIDER" (
    "ORG_ID" VARCHAR(256) NOT NULL,
...
CREATE TABLE IF NOT EXISTS "DP_VIEW_LABELS" (
...
CREATE TABLE IF NOT EXISTS "DP_API_METADATA" (
    ...
    "TAGS" VARCHAR(256),
    "TOKEN_BASED_SUBSCRIPTION_ENABLED" BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE TABLE IF NOT EXISTS "DP_API_LABELS" (
    "ID" VARCHAR(256) NOT NULL,
    "LABEL_ID" VARCHAR(256) NOT NULL,
    "API_ID" VARCHAR(256) NOT NULL,
    "ORG_ID" VARCHAR(256) NOT NULL,
...
CREATE TABLE IF NOT EXISTS "DP_SUBSCRIPTION_POLICY" (
    "ORG_ID" VARCHAR(256) NOT NULL,
...
CREATE TABLE IF NOT EXISTS "DP_APPLICATION" (
...
CREATE TABLE IF NOT EXISTS "DP_API_SUBSCRIPTION" (
...
CREATE TABLE IF NOT EXISTS "DP_APP_KEY_MAPPING" (
...
CREATE TABLE IF NOT EXISTS "DP_SDK_JOB" (
...
CREATE INDEX IF NOT EXISTS idx_metadata_search ON "DP_API_METADATA" USING gin(to_tsvector('english', "METADATA_SEARCH"));
...
-- Additive column migrations (already idempotent)
ALTER TABLE "DP_SUBSCRIPTION_POLICY"
    ADD COLUMN IF NOT EXISTS "PRICING_MODEL" VARCHAR(256),
    ADD COLUMN IF NOT EXISTS "CURRENCY" VARCHAR(256),
    ADD COLUMN IF NOT EXISTS "BILLING_PERIOD" VARCHAR(256),
    ADD COLUMN IF NOT EXISTS "FLAT_AMOUNT" DECIMAL,
    ADD COLUMN IF NOT EXISTS "UNIT_AMOUNT" DECIMAL,
    ADD COLUMN IF NOT EXISTS "PRICING_METADATA" JSONB;

ALTER TABLE "DP_API_SUBSCRIPTION_POLICY"
    ADD COLUMN IF NOT EXISTS "BILLING_METER_ID" VARCHAR(128);

ALTER TABLE "DP_API_SUBSCRIPTION"
    ADD COLUMN IF NOT EXISTS "BILLING_CUSTOMER_ID" VARCHAR(256),
    ADD COLUMN IF NOT EXISTS "BILLING_SUBSCRIPTION_ID" VARCHAR(256),
    ADD COLUMN IF NOT EXISTS "PAYMENT_PROVIDER" VARCHAR(64),
    ADD COLUMN IF NOT EXISTS "PAYMENT_STATUS" VARCHAR(64),
    ADD COLUMN IF NOT EXISTS "CHECKOUT_SESSION_ID" VARCHAR(256);

CREATE INDEX IF NOT EXISTS "IDX_DP_SUB_BILLING_SUB" ON "DP_API_SUBSCRIPTION" ("BILLING_SUBSCRIPTION_ID");
CREATE INDEX IF NOT EXISTS "IDX_DP_SUB_BILLING_CUSTOMER" ON "DP_API_SUBSCRIPTION" ("BILLING_CUSTOMER_ID");
